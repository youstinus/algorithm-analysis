...
namespace Testing
{
    public class Test
    {
            ...
            try
            {
                ...
                switch (which)
                {
                    ...
                    case "4":
                        ...
                        break;
                    case "5":
                        TestInsertionSortArrayOp();
                        break;
                    case "6":
                        TestInsertionSortListOp();
                        break;
                    case "7":
                        TestInsertionSortArrayD();
                        break;
                    case "8":
                        TestInsertionSortListD();
                        break;
                    case "9":
                        TestHashTableSearchOp();
                        break;
                    case "10":
                        TestHashTableSearchD();
                        break;
                    default:
                        throw new InvalidOperationException("Ending tests");
                }
        ...
        private void TestMergeSortArrayOp()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);

            var title = "\n MERGE SORT ARRAY RAM";
            ...
            {
                var dataArray = new MergeSortOP.DataArray(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                ...
                if (!sortedArray.CheckSorted())
                    throw new InvalidOperationException("Merge sort array ram is incorrect");
            }
        ...
        private void TestMergeSortListOp()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);
            ...
            foreach (var count in _counts)
            {
                var dataList = new MergeSortOP.DataList(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                ...
                if (!sortedList.CheckSorted())
                    throw new InvalidOperationException("Merge sort list ram is incorrect");
            }
        ...
        private void TestMergeSortArrayD()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);

            var title = "\n MERGE SORT ARRAY DISK";
            var headers = $" {"N",10}{"TIME",16}{"OPERATIONS",22}\n";
            Console.WriteLine(title);
            Console.WriteLine(headers);
            _output.WriteLine(title);
            ...
            foreach (var count in _counts)
            {
                var dataArray = new MergeSortD.DataArray(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                var sortedArray = dataArray.MergeSortArray();
                var timeString = _timer.Stop();
                ...
                if (!sortedArray.CheckSorted())
                    throw new InvalidOperationException("Merge sort array disk is incorrect");
        ...
        private void TestMergeSortListD()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);

            var title = "\n MERGE SORT LIST DISK";
            var headers = $" {"N",10}{"TIME",16}{"OPERATIONS",22}\n";
            Console.WriteLine(title);
            Console.WriteLine(headers);
            _output.WriteLine(title);
            ...
            foreach (var count in _counts)
            {
                var dataList = new MergeSortD.DataList(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                var sortedList = dataList.MergeSortList();
                var timeString = _timer.Stop();
                ...
                if (!sortedList.CheckSorted())
                    throw new InvalidOperationException("Insertion sort list disk is incorrect");
        ...
        private void TestInsertionSortArrayOp()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);

            var title = "\n INSERTION SORT ARRAY RAM";
            var headers = $" {"N",10}{"TIME",16}{"OPERATIONS",22}\n";
            Console.WriteLine(title);
            Console.WriteLine(headers);
            _output.WriteLine(title);
            ...
            foreach (var count in _counts)
            {
                var dataArray = new InsertionSortOP.DataArray(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                ...
                if (!dataArray.CheckSorted())
                    throw new InvalidOperationException("Insertion sort array ram is incorrect");
            }
        ...
        private void TestInsertionSortListOp()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);

            var title = "\n INSERTION SORT LIST RAM";
            var headers = $" {"N",10}{"TIME",16}{"OPERATIONS",22}\n";
            Console.WriteLine(title);
            Console.WriteLine(headers);
            _output.WriteLine(title);
            ...
            foreach (var count in _counts)
            {
                var dataList = new InsertionSortOP.DataList(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                ...
                if (!dataList.CheckSorted())
                    throw new InvalidOperationException("Insertion sort list ram is incorrect");
            }
        ...
        private void TestInsertionSortArrayD()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);

            var title = "\n INSERTION SORT ARRAY DISK";
            var headers = $" {"N",10}{"TIME",16}{"OPERATIONS",22}\n";
            Console.WriteLine(title);
            Console.WriteLine(headers);
            _output.WriteLine(title);
            ...
            foreach (var count in _counts)
            {
                var dataArray = new InsertionSortD.DataArray(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                dataArray.InsertionSortArray();
                var timeString = _timer.Stop();
                ...
                if (!dataArray.CheckSorted())
                    throw new InvalidOperationException("Insertion sort array disk is incorrect");
        ...
        private void TestInsertionSortListD()
        {
            var prep = Helper.GenerateNumbersArray(_counts.Last(), _seed);

            var title = "\n INSERTION SORT LIST DISK";
            var headers = $" {"N",10}{"TIME",16}{"OPERATIONS",22}\n";
            Console.WriteLine(title);
            Console.WriteLine(headers);
            _output.WriteLine(title);
            ...
            foreach (var count in _counts)
            {
                var dataList = new InsertionSortD.DataList(prep.Take(count).ToArray());
                Counter.Reset();
                _timer.Start();
                dataList.InsertionSortList();
                var timeString = _timer.Stop();
        ...
        private void TestHashTableSearchOp()
        {
            ...
            foreach (var count in _counts)
            {
                var table = new HashTableSearchOP.HashTable(count);
                Counter.Reset();
                _timer.Start();
        ...
        private void TestHashTableSearchD()
        {
            var keys = Helper.GetStringList(_counts.Last());
            var values = Helper.GetStringList(_counts.Last());
            ...
            foreach (var count in _counts)
            {
                var table = new HashTableSearchD.HashTable(count);
                Counter.Reset();
                _timer.Start();
                for (var i = 0; i < count; i++)
                {
