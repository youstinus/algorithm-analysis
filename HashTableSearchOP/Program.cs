...
{
    /* 1) kiek šakų/grandinėlių; 2) du nurodyti elementai yra vienoje
šakoje/grandinėlėje ir kokios jų palyginimo/maišos funkcijų reikšmės.*/
    class Program
    {
        ...
        private static void StartHashTable(int n, List<string> keys, List<string> values)
        {
            ...
            Console.WriteLine($" Table size: {table.GetTableSize()}. Element count: {table.GetElementCount()}. Used Table spaces: {table.GetUsedSpacesCount()}.");
            Console.WriteLine($" First chain with more than one element and their key hashes:\n {table.MoreThanOneItemHash()}");
        }
